function IDBmethods(promiseFromDb, _objectStore) {
    ...
    return {
        openIDB(dbname, vr, calback) {
            if (typeof idb !== "undefined") {
                return idb.open(dbname, vr, calback)
            } else {
                console.error("Typeof idb is not an object, please make sure you've loaded the idb library first.");
            }
        ...
        updateProperty(key, property, value) {
            return promiseFromDb.then(db => {
                ...
                for (let newDataProperty of _properties) {
